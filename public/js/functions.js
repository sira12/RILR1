$(document).ready(function () {
    ...
    $("#search_codigo").autocomplete({
        ...
        select: function (event, ui) {
            ...
            $("#detalle_actividad").val(
                ui.item.label + " - " + ui.item.actividad
            );
            return false;
        },
    });
    ...
    $("#search_descripcion").autocomplete({
        ...
        select: function (event, ui) {
            ...
            $("#detalle_actividad").val(
                ui.item.actividad + " - " + ui.item.label
            );
            return false;
        },
    });
    ...
    $("#search_pais_insumo").keyup(function () {
        if ($("#search_pais_insumo").val().length < 1) {
            $("#id_pais_insumo").val("");
    ...
    $("#search_provincia_insumo").keyup(function () {
        if ($("#search_provincia_insumo").val().length < 1) {
            $("#id_provincia_insumo").val("");
            $("#search_localidad_insumo").val("");
            $("#id_localidad_insumo").val("");
            ...
            if ($("#id_provincia_insumo").val() !== "") {
                $.ajax({
                    ...
                    data: {
                        _token: $('meta[name="csrf-token"]').attr("content"),
                        search: request.term,
                        id_prov: $("#id_provincia_insumo").val(),
                    },
                    success: function (data) {
                        response(data);
                    },
    ...
    $("#search_localidad_insumo").keyup(function () {
        if ($("#search_localidad_insumo").val().length < 1) {
            $("#id_localidad_insumo").val("");
        }
    });
    //#########################combustible$#####################
            ...
            if ($("#id_pais_combustible").val() !== "") {
                $.ajax({
                    ...
                    data: {
                        _token: $('meta[name="csrf-token"]').attr("content"),
                        search: request.term,
                        id_pais: $("#id_pais_combustible").val(),
                    },
                    success: function (data) {
                        response(data);
                    },
    ...
    $("#search_localidad_combustible").autocomplete({
        source: function (request, response) {
            // Fetch data
            if ($("#id_provincia_combustible").val() !== "") {
                $.ajax({
                    url: "/localidades",
                    type: "post",
                    dataType: "json",
                    data: {
                        _token: $('meta[name="csrf-token"]').attr("content"),
                        search: request.term,
                        id_prov: $("#id_provincia_combustible").val(),
                    },
                    success: function (data) {
                        response(data);
                    },
            ...
            if ($("#id_provincia_otros").val() !== "") {
                $.ajax({
                    ...
                    data: {
                        _token: $('meta[name="csrf-token"]').attr("content"),
                        search: request.term,
                        id_prov: $("#id_provincia_otros").val(),
                    },
                    success: function (data) {
                        response(data);
                    },
    ...
    table = $(".yajra-datatable").DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                ...
                data: "fecha_inicio",
                render: function (data, type, row, meta) {
                    return row.fecha_inicio.replace(
                        /^(\d{4})-(\d{2})-(\d{2})$/g,
                        "$3-$2-$1"
                    );
                },
...
function cargar_tabla_materia() {
    ...
    table.destroy();
    //$('.yajra-datatable').empty();
    var id_asignacion_producto = $(
        "#id_rel_industria_actividad_materia_prima"
    ).val();
    var periodo_fiscal = $("#anio_periodo_fiscal").val();
...
function cargar_clasif_ingresos() {
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#clasif_ingreso_2").append(
                '<tr role="row" class="text-left"><th>Categoria</th><th>Nivel de Ingresos</th><th>Corresponde</th></tr></thead>'
            );
            $(response).each(function (i, v) {
                $("#clasif_ingreso").append(
                    '<tr role="row" class="text-left">' +
                        ...
                        '   <input type="radio" id="categoria_ingresos_' +
                        v.id_categoria_ingreso +
                        '" name="categoria_ingresos" value="' +
                        v.id_categoria_ingreso +
                        '" class="custom-control-input c_fac">' +
                        ' <label class="custom-control-label" for="categoria_ingresos_' +
                        v.id_categoria_ingreso +
                        '"></label>' +
                        "</div>" +
                        " </td>" +
    ...
    $.ajax({
        ...
        success: function (response) {
            $(response.periodo_fiscal).each(function (i, v) {
                $("#anio_periodo_fiscal").append(
                    '<option value="' + v.anio + '">' + v.anio + "</option>"
                );
            });
            ...
            $("#nombre_industria_fantasia").text(
                response.industria.nombre_de_fantasia
            );
            $("#nombre_de_fantasia").val(response.industria.nombre_de_fantasia);
            $("#fecha_actividad_industria").val(
                moment(response.industria.fecha_inicio).format("DD-MM-YYYY")
            );
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(response).each(function (i, v) {
                $("#clasif_venta").append(
                    '<option value="' +
                        v.id_clasificacion_ventas +
                        '">' +
                        v.clasificacion_ventas +
                        "</option>"
                );
            });
        },
...
function getPaisesVentas() {
    $.ajax({
        ...
        success: function (response) {
            ...
            //$('#ventas_provincias').append('<option value="">-- SELECCIONE --</option>');
            $(response).each(function (i, v) {
                $("#ventas_paises").append(
                    '<option value="' + v.id_pais + '">' + v.pais + "</option>"
                );
            });
        },
    });
            ...
            $(response).each(function (i, v) {
                $("#p_o").append(
                    '<tr role="row" class="odd">' +
                        '<td><input type="hidden" name="id_condicion_laboral[]" value="' +
...
function getunidades() {
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#medida_combustible").append(
                '<option value="">' + "--SELECCIONE--" + "</option>"
            );
            $("#medida_insumo option").remove();
            $("#medida_insumo").append(
                '<option value="">' + "--SELECCIONE--" + "</option>"
            );
            $(response).each(function (i, v) {
                // indice, valor
...
function getMotivo() {
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#motivo_importacion_insumo").append(
                '<option value="">' + "--SELECCIONE--" + "</option>"
            );
            $("#motivo_importacion_combustible option").remove();
            $("#motivo_importacion_combustible").append(
                '<option value="">' + "--SELECCIONE--" + "</option>"
            );
            $("#motivo_importacion_otros option").remove();
            $("#motivo_importacion_otros").append(
                '<option value="">' + "--SELECCIONE--" + "</option>"
            );
            $(response).each(function (i, v) {
                $("#motivo_importacion_insumo").append(
        ...
        success: function (response) {
            ...
            $(response).each(function (i, v) {
                $(".table_certificado_modal").append(
                    '<tr role="row" class="odd">' +
                        '<td><input type="hidden" name="id_certificado[]" id="id_certificado_' +
                        v.id_certificado +
                        '" value="' +
                        v.id_certificado +
                        '"><label>' +
                        ...
                        ']" id="name1_' +
                        v.id_certificado +
                        '" value="NO POSEE" class="custom-control-input" onclick="ProcesarCertificado(\'NO POSEE\',' +
                        v.id_certificado +
                        ');">' +
                        '<label class="custom-control-label" for="name1_' +
                        v.id_certificado +
                        '"></label>' +
                        "</div></td>" +
                        '<td class="text-center"><div class="custom-control custom-radio custom-control-inline">' +
                        ...
                        ']" id="name2_' +
                        v.id_certificado +
                        '" value="EN TRAMITE" class="custom-control-input" onclick="ProcesarCertificado(\'EN TRAMITE\',' +
                        v.id_certificado +
                        ');">' +
                        '<label class="custom-control-label" for="name2_' +
                        v.id_certificado +
                        '"></label>' +
                        "</div></td>" +
                        '<td class="text-center"><div class="custom-control custom-radio custom-control-inline">' +
                        ...
                        ']" id="name3_' +
                        v.id_certificado +
                        '" value="POSEE" class="custom-control-input" onclick="ProcesarCertificado(\'POSEE\',' +
                        v.id_certificado +
                        ');">' +
                        '<label class="custom-control-label" for="name3_' +
                        v.id_certificado +
                        '"></label>' +
                        "</div></td>" +
                        ' <td class="text-center"><input type="text" class="form-control certificado" name="inicio_certificado[' +
                        i +
                        ']" id="inicio_certificado_' +
                        v.id_certificado +
                        '" placeholder="Ingrese Fecha Inicial" autocomplete="off" style="width:100%;height:40px;background:#f0f9fc;border-radius:5px 5px 5px 5px;"  disabled="" title="Ingrese Fecha Inicial" required="" aria-required="true"></td>' +
                        ' <td class="text-center"><input type="text" class="form-control certificado" name="fin_certificado[' +
                        i +
                        ']" id="fin_certificado_' +
                        v.id_certificado +
                        '" placeholder="Ingrese Fecha Final" autocomplete="off" style="width:100%;height:40px;background:#f0f9fc;border-radius:5px 5px 5px 5px;" disabled="" title="Ingrese Fecha Final" required="" aria-required="true"></td>' +
                        " </tr>"
                );
            });
        },
        ...
        success: function (response) {
            ...
            $(response).each(function (i, v) {
                $("#tbody_sa_modal").append(
                    "<tr>" +
                        '<td><input type="hidden" name="id_sistema_de_calidad[]"  value="' +
                        v.id_sistema_de_calidad +
                        '"><label>' +
                        ...
                        ']" id="name4_' +
                        v.id_sistema_de_calidad +
                        '" value="NO POSEE" class="custom-control-input" onclick="ProcesarSistema(\'NO POSEE\',' +
                        v.id_sistema_de_calidad +
                        ');">' +
                        '<label class="custom-control-label" for="name4_' +
                        v.id_sistema_de_calidad +
                        '"></label>' +
                        "</div></td>" +
                        '<td class="text-center"><div class="custom-control custom-radio custom-control-inline">' +
                        ...
                        ']" id="name5_' +
                        v.id_sistema_de_calidad +
                        '" value="EN TRAMITE" class="custom-control-input" onclick="ProcesarSistema(\'EN TRAMITE\',' +
                        v.id_sistema_de_calidad +
                        ');">' +
                        '<label class="custom-control-label" for="name5_' +
                        v.id_sistema_de_calidad +
                        '"></label>' +
                        " </div></td>" +
                        ' <td class="text-center"><div class="custom-control custom-radio custom-control-inline">' +
                        ...
                        ']" id="name6_' +
                        v.id_sistema_de_calidad +
                        '" value="POSEE" class="custom-control-input" onclick="ProcesarSistema(\'POSEE\',' +
                        v.id_sistema_de_calidad +
                        ');">' +
                        '<label class="custom-control-label" for="name6_' +
                        v.id_sistema_de_calidad +
                        '"></label>' +
                        " </div></td>" +
                        ' <td class="text-center"><input type="text" class="form-control calidad" name="inicio_sistema[' +
                        i +
                        ']" id="inicio_sistema_' +
                        v.id_sistema_de_calidad +
                        '" placeholder="Ingrese Fecha Inicial" autocomplete="off" style="width:100%;height:40px;background:#f0f9fc;border-radius:5px 5px 5px 5px;" title="Ingrese Fecha Inicial" required="" aria-required="true" disabled="disabled"></td>' +
                        '<td class="text-center"><input type="text" class="form-control calidad" name="fin_sistema[' +
                        i +
                        ']" id="fin_sistema_' +
                        v.id_sistema_de_calidad +
                        '" placeholder="Ingrese Fecha Final" autocomplete="off" style="width:100%;height:40px;background:#f0f9fc;border-radius:5px 5px 5px 5px;" title="Ingrese Fecha Final" required="" aria-required="true" disabled="disabled"></td>' +
                        "</tr>"
                );
            });
        },
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(response).each(function (i, v) {
                $("#tbody_promo").append(
                    "<tr>" +
                        '<td><input type="hidden" name="id_promocion_industrial[]"  value="' +
                        v.id_promocion_industrial +
                        '"><label>' +
                        ...
                        ']" id="name7_' +
                        v.id_promocion_industrial +
                        '" value="NO POSEE" class="custom-control-input" onclick="ProcesarPromocion(\'NO POSEE\',' +
                        v.id_promocion_industrial +
                        ');">' +
                        '<label class="custom-control-label" for="name7_' +
                        v.id_promocion_industrial +
                        '"></label>' +
                        "</div></td>" +
                        '<td class="text-center"><div class="custom-control custom-radio custom-control-inline">' +
                        ...
                        ']" id="name8_' +
                        v.id_promocion_industrial +
                        '" value="EN TRAMITE" class="custom-control-input" onclick="ProcesarPromocion(\'EN TRAMITE\',' +
                        v.id_promocion_industrial +
                        ');">' +
                        '<label class="custom-control-label" for="name8_' +
                        v.id_promocion_industrial +
                        '"></label>' +
                        " </div></td>" +
                        ' <td class="text-center"><div class="custom-control custom-radio custom-control-inline">' +
                        ...
                        ']" id="name9_' +
                        v.id_promocion_industrial +
                        '" value="POSEE" class="custom-control-input" onclick="ProcesarPromocion(\'POSEE\',' +
                        v.id_promocion_industrial +
                        ');">' +
                        '<label class="custom-control-label" for="name9_' +
                        v.id_promocion_industrial +
                        '"></label>' +
                        " </div></td>" +
                        ' <td class="text-center"><input type="text" class="form-control calendario2" name="inicio_promocion[' +
                        i +
                        ']" id="inicio_promocion_' +
                        v.id_promocion_industrial +
                        '" placeholder="Ingrese Fecha Inicial" autocomplete="off" style="width:100%;height:40px;background:#f0f9fc;border-radius:5px 5px 5px 5px;" title="Ingrese Fecha Inicial" required="" aria-required="true" disabled="disabled"></td>' +
                        '<td class="text-center"><input type="text" class="form-control calendario2" name="fin_promocion[' +
                        i +
                        ']" id="fin_promocion_' +
                        v.id_promocion_industrial +
                        '" placeholder="Ingrese Fecha Final" autocomplete="off" style="width:100%;height:40px;background:#f0f9fc;border-radius:5px 5px 5px 5px;" title="Ingrese Fecha Final" required="" aria-required="true" disabled="disabled"></td>' +
            ...
            $(response).each(function (i, v) {
                $("#tbody_sector").append(
                    '<tr role="row" class="text-left">' +
                        ...
                        '<input type="checkbox" name="sectores[' +i +']" id="s' +v.id_economia_del_conocimiento_sector +'" value="' +v.id_economia_del_conocimiento_sector +
                        '" class="custom-control-input cb_sector">' +
                        '  <label class="custom-control-label" for="s' +
                        v.id_economia_del_conocimiento_sector +
                        '"></label>' +
                        "  </div>" +
                        "  </td>" +
                        " </tr>"
            ...
            $(response).each(function (i, v) {
                $("#tbody_perfil").append(
                    '<tr role="row" class="text-left">' +
                        ...
                        " <td>" +
                        ' <div class="custom-control custom-radio custom-control-inline">' +
                        '<input type="checkbox" name="perfiles[' +
                        i +
                        ']" id="p' +
                        v.id_economia_del_conocimiento_perfil +
                        '" value="' +
                        v.id_economia_del_conocimiento_perfil +
                        '" class="custom-control-input cb_perfil">' +
...
function alertDj() {
    swal(
        "Oops",
        "Está seccion está en desarrollo, disculpe las molestias.",
        "warning"
    );
}
